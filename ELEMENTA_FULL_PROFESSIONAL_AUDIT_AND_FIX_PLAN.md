...
**T5 — Double-Opt-in Newsletter (P2)** ✅ **ERLEDIGT (2026-07-04)**
...
Umsetzung: newsletter-Collection um `confirmedAt` + `token` erweitert (`scripts/provision-newsletter-doi.mjs`). Mailversand via **nodemailer** (`src/lib/mail.ts`, SMTP über Env `SMTP_HOST/PORT/USER/PASS/FROM`). POST `/api/newsletter` legt Eintrag unbestätigt an + sendet Bestätigungslink; **kein stiller Fake-Erfolg** — ohne konfiguriertes SMTP ehrlicher 503, bei Sendefehler wird der neue Eintrag zurückgerollt. GET `/api/newsletter/confirm` prüft id+token, setzt `confirmedAt`, löscht Token, leitet auf `/newsletter/bestaetigt?state=…` (Landing mit ok/already/invalid/error). UI-Meldung ehrlich („Bestätigungs-Mail geschickt — bitte Link klicken"). **Cron** `scripts/newsletter-cleanup.mjs` (So 3:30, absoluter Pfad, lädt Key selbst aus `.env.local`) löscht unbestätigte >30 Tage. Datenschutzseite um Double-Opt-in-Absatz ergänzt. E2E-nachgewiesen (`scripts/test-newsletter.mjs` gegen lokalen SMTP-Catcher, 9/9): Signup→Mail→Confirm→confirmedAt gesetzt→erneut „already"→Cleanup-Query greift. ⚠️ Für echten Versand muss Belkis SMTP-Zugangsdaten in `.env.local` eintragen (IONOS: `smtp.ionos.de:587`); bis dahin meldet der Newsletter ehrlich „derzeit nicht eingerichtet". ⚠️ `NEXT_PUBLIC_SITE_URL` wird build-time eingebacken — nach Änderung neu bauen.

**T6 — Mock-Fallback entschärfen (P2)** ✅ **ERLEDIGT (2026-07-04)**
...
- [x] **T5:** Double-Opt-in-Newsletter (2026-07-04: Bestätigungsmail + Confirm + 30-Tage-Cron, 9/9 E2E)
- [x] **T6:** Kein Mock-Fallback mehr — ehrlicher Zustand bei DB-Ausfall (2026-07-04: E2E-verifiziert, Fake-Daten entfernt)
